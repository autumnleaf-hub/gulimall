...
public class GenerateCode {
    ...
    String url = "jdbc:mysql://192.168.113.130:3306/gulimall_sms?useUnicode=true&characterEncoding=utf-8&useSSL=false&serverTimezone=Asia/Shanghai";
    @Value("${spring.datasource.username}")
    String username = "root";
    @Value("${spring.datasource.password}")
    String password = "root";
    final String author = "fzy";                   // 作者名
    String tableNamePattern = "sms_";                  // 通过模糊匹配的方式添加表名
    String[] tables = {"sms_.*"};                  // 额外添加表
    String[] tablePrefix = new String[]{"sms_"};       // 过滤表前缀
    String tableSuffix = "";                       // 过滤表后缀
    IdType entityIdType = IdType.ASSIGN_ID;        // 生成实体映射的主键填充方式
        ...
        FastAutoGenerator.create(url, username, password)
                ...
                .packageConfig(builder -> {
                    builder.parent(parentPackage) // 设置父包名
                            ...
                            .pathInfo(Collections.singletonMap(OutputFile.xml, ".\\resources"))
                    ;
                })
                .strategyConfig(builder -> {
                    builder//.likeTable(new LikeTable(tableNamePattern))
